...
public class ScaryMaze extends JPanel implements Runnable, MouseMotionListener {
	...
	public void mouseMoved(MouseEvent e) {
		...
		int deathcolor = -16777216;
		//8. If the mouse falls off the path (if it is on the background)
	 if (mouseColor == deathcolor) { scare ();
	 
	 }
				// call the scare method
		
		//13. If the mouse is on the end color
	int endcolor = -16711889;
	
	if (mouseColor == endcolor) { JOptionPane.showMessageDialog(null, "you won. yay. now do something better with your life.");
	...
	private void scare() {
		...
		AudioClip sound = JApplet.newAudioClip(getClass().getResource("scarybang.wav"));
